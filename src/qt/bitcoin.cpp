...
static QString GetLangTerritory()
{
    ...
    // 1) Mulacoin default language: pt_BR (moeda focada no público brasileiro)
    QString lang_territory = "pt_BR";
    // 2) Language from QSettings (usuário pode trocar em Configurações > Opções > Janela)
    QString lang_territory_qsettings = settings.value("language", "").toString();
    if(!lang_territory_qsettings.isEmpty())
        lang_territory = lang_territory_qsettings;
    // 3) -lang command line argument (maior prioridade, sobrescreve tudo)
...
static const char* qt_argv = "mulacoin-qt";
...
{
    QMessageBox::critical(0, BitcoinGUI::tr("Runaway exception"), BitcoinGUI::tr("A fatal error occurred. Mulacoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
